...
def parse():
    ...
    parser.add_argument(
        ...
        metavar="people",
        type=int,
        help="Quantity of people in the distribution",
        default=400,
    )
    parser.add_argument(
        "-s", metavar="s", type=float, help="constant of Zipf Distribution", default=1.4
    )
    parser.add_argument(
        "--threads",
        metavar="threads",
        type=int,
        help="Number of threads for concurrency",
        default=4,
    )
    parser.add_argument(
        "--max",
        metavar="max_msgs",
        type=int,
        help="Maximum amount of messages from one only author",
        default=15,
    )
    parser.add_argument(
        "--seed", metavar="seed", type=int, help="Number of seeds", default=200
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    pref.add_argument(
        ...
        metavar="w",
        type=int,
        help="Value of weight for each signature",
        default=1,
    )
    pref.add_argument(
        "--fweight",
...
def main_uniform(args):
    ...
    context: UniformZipfContext = UniformZipfContext(
        people=PEOPLE, k=K, max_msg=args.max, s=args.s
    )
...
def main_preferential(args):
    ...
    context: PreferentialContext = PreferentialContext(
        people=PEOPLE,
        k=K,
        max_msg=args.max,
        s=args.s,
