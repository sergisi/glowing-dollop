...
def simulation_first(persons: int, ring_order: int,
    ...
    """
        ...
        :param persons -> int: the number of people in the forum. Every person 
            is named as [0, persons)
        :param ring_order -> int: the number of people to be selected in a post
        :param message_list -> 
            List[int] : List of messages in order. For example [0, 3, 2]
            would send first "0" message, "3" message and "2" message
        :return List[List[int]] : 
            A list of every set of people selected for the message. For
            example, in the first message could have been [3, 4, 0] as the 
            people selected. All the results are appended in a message like 
                [[3, 4, 0]...]. 
            It's returned as a List of Lists instead of a List of Sets for 
            performance.
    """
...
def simulation(persons: int, ring_order: int,
    ...
    """
        ...
        :param persons -> int: the number of people in the forum.
            Every person is named as [0, persons)
        :param ring_order -> int: the number of people to be selected in a post
        :param message_list -> List[int] : List of messages in order. 
            For example [0, 3, 2]
            would send first "0" message, "3" message and "2" message
        :return List[List[int]] : A list of every set of people 
            selected for the message. For example, in the first message 
            could have been [3, 4, 0] as the people selected. All the 
            results are appended in a message like:
                [[3, 4, 0]...]. It's returned as a List of
